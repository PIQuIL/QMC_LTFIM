...
function all_vars(B::LogBinner{T,N}) where {T,N}
    [var(B, lvl) for lvl in 1:N if count(B.accumulators[lvl]) > 1]
end
...
function all_varNs(B::LogBinner{T,N}) where {T,N}
    [varN(B, lvl) for lvl in 1:N if count(B.accumulators[lvl]) > 1]
end
...
function all_means(B::LogBinner{T,N}) where {T,N}
    [mean(B, lvl) for lvl in 1:N if count(B.accumulators[lvl]) > 1]
end
...
function all_taus(B::LogBinner{T,N}) where {T,N}
    [tau(B, lvl) for lvl in 1:N if count(B.accumulators[lvl]) > 1]
end
...
function _reliable_level(B::LogBinner{T,N})::Int64 where {T,N}
    ...
    i = findlast(x -> count(x) >= 32, B.accumulators)
    something(i, 1)
end
...
function all_std_errors(B::LogBinner{T,N}) where {N, T}
    [std_error(B, lvl) for lvl in 1:N if count(B.accumulators[lvl]) > 1]
end
