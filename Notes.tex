...
\begin{subequations} \label{eq:decomposition_ryd}
    \begin{align}
        ...
        H_{1,a} =& \frac{\Omega}{2} \\
        H_{1,b} =& V_{ij} \ketbra{1_i 1_j}{1_i 1_j} - \delta_b \left(\ketbra{1}{1}_i + \ketbra{1}{1}_j\right) + C
    \end{align}
\end{subequations}
$C = 2\delta_b$ is a constant shift that is needed in order to alleviate sign problems.
...
\begin{subequations} \label{eq:mat_elems_ryd}
    \begin{align}
        ...
        W_1 \equiv \matrixel{\uparrow \uparrow}{H_{1,b}}{\uparrow \uparrow} =& V_{ij} - 2\delta_b + C = V_{ij}\\
        W_2 \equiv \matrixel{\downarrow \downarrow}{H_{1,b}}{\downarrow \downarrow} =& C = 2\delta_b \\
        W_3 \equiv \matrixel{\downarrow \uparrow}{H_{1,b}}{\downarrow \uparrow} = \matrixel{\uparrow \downarrow}{H_{1,b}}{\uparrow \downarrow} =& -\delta_b + C = \delta_b
    \end{align}
\end{subequations}
...
\begin{enumerate}
    ...
    \begin{align*}
        P_{[1,a]_p / [1,b]_p \rightarrow [0,0]_p} =& \mathrm{min}\left( \frac{W^\prime(\alpha(p), S_M)}{ W(H_{[1,a]_p} \text{ or } H_{[1,b]_{p,\uparrow\uparrow}} \text{ or } H_{[1,b]_{p,\downarrow\downarrow}} \text{ or } H_{[1,b]_{p,\uparrow\downarrow / \downarrow\uparrow}}) }, 1 \right) \\
        =& \mathrm{min}\left( \frac{W^\prime(\alpha(p), S_M)}{NW(H_{[1,a]_p}) + \sum_{ij} V_{ij} + N_b(W(H_{[1,b]_{p,\downarrow\downarrow}}) + W(H_{[1,b]_{p,\uparrow\downarrow / \downarrow\uparrow}})) }, 1 \right) \\
        =& \mathrm{min}\left(\frac{M-n+1}{\beta \left(Nh + \sum_{ij} V_{ij} + 4N_b\delta_b\right) },1\right).
    \end{align*}
    The factor of $N$ or $N_b$ in the denominator is from the fact that where the operator $H_{1,a}$ or $H_{1,b}$ is when we remove it does not matter. 
    \item If the unity operator $H_{0,0}$ is encountered, decide whether or not to accept inserting a diagonal operator ($n \rightarrow n + 1$) with the probability 
    \begin{align}
        P_{H_{[0,0]_p} \rightarrow H_{[1,a]_p} / H_{[1,b]_p}} =& \mathrm{min}\left(\frac{\beta \left(Nh + \sum_{ij} V_{ij} + 4N_b\delta_b\right) }{M-n},1\right)
    \end{align}
    \item If it was decided to insert a diagonal operator, choose the operator to insert with probabilities
    \begin{subequations} \label{eq:heat_bath_1_ryd}
        \begin{align} 
            P_{H_{[1,a]_p}} = & \frac{N\frac{\Omega}{2}}{N\frac{\Omega}{2} + \sum_{ij} V_{ij} + 4N_b\delta_b}, \\
            P_{H_{[1,b]_p}} = & \frac{\sum_{ij} V_{ij} + 4N_b\delta_b}{N\frac{\Omega}{2} + N_b\left(W_1 + W_2 + 2W_3\right)}.
        \end{align}
    \end{subequations}
