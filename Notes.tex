...
\end{align}
Upon identifying $\ket{g},\ket{r}$ with eigenstates of $\sigma^z$ ($\ket{0},\ket{1}$), we can translate this Hamiltonian into spin language, where it becomes nearly analogous to a transverse field Ising model with a longitudinal field.
%\begin{align}
...
\begin{subequations}
    \begin{align}
        H = & \Omega \sum_{i = 1}^N \prod_{\langle i,j \rangle}\left(1-\frac{1}{2}\left(\sigma_i^z + \mathbb{1} \right)\right) \sigma^x_i - \delta \sum_{i=1}^n n_i + \sum_{\expval{i,j}} V_{ij} n_i n_j
    \end{align}
\end{subequations}
...
\begin{enumerate}
    ...
    Specifically, 
    \begin{align*}
        P_{\uparrow \uparrow} =& \frac{W_1}{W_1 + W_2 + W_3} \\
        P_{\downarrow \downarrow} =& \frac{W_2}{W_1 + W_2 + W_3} \\
        P_{\uparrow \downarrow / \downarrow \uparrow} =& \frac{W_3}{W_1 + W_2 + W_3}. 
    \end{align*}
    ...
    {\color{red} {\bf THIS GIVES A WORSE ACCEPTANCE RATIO:}
